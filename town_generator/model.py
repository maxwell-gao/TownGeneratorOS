...
class Model:
        ...
        while retries < max_retries:
            try:
                ...
                if retries >= max_retries:
                    raise RuntimeError(f"Failed to build city after {max_retries} attempts: {e}")
    ...
    def _optimize_junctions(self):
        ...
        patches_to_optimize = self.inner if self.citadel is None else self.inner + [self.citadel]
        ...
        for w in wards_to_clean:
            ...
            while i < len(w.shape.vertices):
                ...
                dup_idx = w.shape.vertices.index(v, i + 1) if v in w.shape.vertices[i+1:] else -1
                if dup_idx != -1:
    ...
    def _build_streets(self):
        ...
        for gate in self.gates:
            end = (self.plaza.shape.min(lambda v: v.distance(gate)) 
                  if self.plaza else self.center)
    ...
    def _tidy_up_roads(self):
        ...
        def cut_to_segments(street):
            ...
            for i in range(1, len(street.vertices)):
                ...
                if self.plaza and (self.plaza.shape.contains(v0) and self.plaza.shape.contains(v1)):
                    v0 = v1
                    continue
    ...
    def find_circumference(self, wards):
        ...
        for w1 in wards:
            w1.shape.for_edge(lambda a, b: self._process_circumference_edge(a, b, w1, wards, A, B))
    ...
    def to_dict(self):
        ...
        return {
            ...
            "wall": {
                ...
                "towers": [t.to_dict() for t in (self.wall.towers if self.wall else [])]
            } if self.wall else None,
            "citadel": {
                "shape": self.citadel.shape.to_dict() if self.citadel else None,
                "wall": {
                    "shape": self.citadel.ward.wall.shape.to_dict() if (self.citadel and self.citadel.ward and hasattr(self.citadel.ward, 'wall')) else None,
                    "gates": [g.to_dict() for g in (self.citadel.ward.wall.gates if (self.citadel and self.citadel.ward and hasattr(self.citadel.ward, 'wall')) else [])],
                    "towers": [t.to_dict() for t in (self.citadel.ward.wall.towers if (self.citadel and self.citadel.ward and hasattr(self.citadel.ward, 'wall')) else [])]
                } if (self.citadel and self.citadel.ward and hasattr(self.citadel.ward, 'wall')) else None
            } if self.citadel else None,

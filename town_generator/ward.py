...
class Ward:
    ...
    def get_city_block(self):
        ...
        self.patch.shape.for_edge(lambda v0, v1: self._process_edge(v0, v1, inner_patch, inset_dist))
    ...
    def _process_edge(self, v0, v1, inner_patch, inset_dist):
        ...
        if (self.model.wall is not None and 
            self.model.wall.borders_by(self.patch, v0, v1)):
            ...
            if not on_street:
                for street in self.model.arteries:
                    if street.contains(v0) and street.contains(v1):
                        ...
                        break
            inset_dist.append(
                (self.MAIN_STREET if on_street else (self.REGULAR_STREET if inner_patch else self.ALLEY)) / 2
            )
    ...
    def filter_outskirts(self):
        ...
        def add_edge(v1, v2, factor=1.0):
            ...
            dy = v2.y - v1.y
            distances = {}
            d = self.patch.shape.max(lambda v: distances.setdefault(v, 
                (0 if (v == v1 or v == v2) else distance2line(v1.x, v1.y, dx, dy, v.x, v.y)) * factor))
            populated_edges.append({
                'x': v1.x, 'y': v1.y, 'dx': dx, 'dy': dy, 
                'd': distances.get(d, 0) if d else 0
            })
        
        self.patch.shape.for_edge(lambda v1, v2: self._process_outskirts_edge(v1, v2, add_edge))
        ...
        # Filter buildings
        self.geometry = [b for b in self.geometry if self._should_keep_building(b, populated_edges, density)]
    ...
    def _should_keep_building(self, building, populated_edges, density):
        ...
        for edge in populated_edges:
            for v in building.vertices:
                d = distance2line(edge['x'], edge['y'], edge['dx'], edge['dy'], v.x, v.y)
    ...
    @staticmethod
    def create_alleys(poly, min_sq, grid_chaos, size_chaos, empty_prob=0.04, split=True, depth=0):
        ...
        angle_spread = math.pi / 6 * grid_chaos * (1 if poly.square >= min_sq * 4 else 0)
        ...
        for half in halves:
            if len(half.vertices) < 3:
                continue
            if half.square < min_sq * math.pow(2, 4 * size_chaos * (Random.float() - 0.5)):
                if not Random.bool(empty_prob):
                    buildings.append(half)
            else:
                should_split = half.square > min_sq / (Random.float() * Random.float() + 0.1)
                buildings.extend(Ward.create_alleys(half, min_sq, grid_chaos, size_chaos, empty_prob, should_split, depth + 1))
    ...
    def create_ortho_building(poly, min_block_sq, fill):
        ...
        def slice(poly, c1, c2):
            ...
            c = c1 if abs(scalar(v.x, v.y, c1.x, c1.y)) < abs(scalar(v.x, v.y, c2.x, c2.y)) else c2
...
class CommonWard(Ward):
    ...
    def create_geometry(self):
        ...
        self.geometry = Ward.create_alleys(block, self.min_sq, self.grid_chaos, self.size_chaos, self.empty_prob)
...
class Castle(Ward):
    ...
    def __init__(self, model, patch):
        ...
        reserved = [v for v in patch.shape.vertices 
    ...
    def create_geometry(self):
        ...
        self.geometry = Ward.create_ortho_building(block, math.sqrt(block.square) * 4, 0.6)
